...
// ─────────────────────────────────────────
// 3b. Novo agendamento → PROFISSIONAL
// ─────────────────────────────────────────
export async function enviarNotificacaoProfissional(agendamento, telefoneProfissional) {
  if (!telefoneProfissional) return { success: false, error: 'Telefone da profissional não informado' }

  const { cliente_nome, cliente_telefone, servico, data, horario, metodo_pagamento } = agendamento
...
_Acesse o painel para mais detalhes._`
  ...
  return sendMessage(telefoneProfissional, msg)
}
